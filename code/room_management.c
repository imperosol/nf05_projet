...
{
    (*disponibilityHour)[HOUR] = h;
    (*disponibilityHour)[MINUTE] = m;
}
...
void exam_of_the_patient (patient* patient, waitingRoom* firstRoom, waitingRoom* secondRoom, examRoom* examRoom,int duration)
{
    ...
    if ((patient->disponibilityHour[HOUR]>examRoom->endOfExamHour[HOUR]) || ((patient->disponibilityHour[HOUR]==examRoom->endOfExamHour[HOUR]) && (patient->disponibilityHour[MINUTE]>examRoom->endOfExamHour[MINUTE])) )
    {
        patient->examStart[patient->examNumber].hour[HOUR] = patient->disponibilityHour[HOUR];
        patient->examStart[patient->examNumber].hour[MINUTE] = patient->disponibilityHour[MINUTE];
        patient->disponibilityHour[HOUR] = patient->disponibilityHour[HOUR] + duration/60;
        patient->disponibilityHour[MINUTE] = patient->disponibilityHour[MINUTE] + duration%60;
    }
    else
    {
        ...
        patient->disponibilityHour[HOUR] = examRoom->endOfExamHour[HOUR] + duration/60;
        patient->disponibilityHour[MINUTE] = examRoom->endOfExamHour[MINUTE] + duration%60;
    }
    if (patient->disponibilityHour[MINUTE]>=60)//Update to the next hour if the number of minutes exceeds 59.
    {
        patient->disponibilityHour[HOUR] += (patient->disponibilityHour[MINUTE])/60;
        patient->disponibilityHour[MINUTE] = (patient->disponibilityHour[MINUTE])%60;
    }
    examRoom->endOfExamHour[HOUR] = patient->disponibilityHour[HOUR];
    examRoom->endOfExamHour[MINUTE] = patient->disponibilityHour[MINUTE];
    patient->examEnd[patient->examNumber].hour[HOUR] = patient->disponibilityHour[HOUR];
    patient->examEnd[patient->examNumber].hour[MINUTE] = patient->disponibilityHour[MINUTE];
